...
w = faithful$waiting
ggplot(NULL, aes(x=w)) + geom_histogram()    ## data 可以设为NULL, x 设为向量w
ggplot(faithful, aes(x=waiting)) +
  geom_histogram(binwidth=5,fill="white", colour="black",boundary = 0.5)
...
ggplot(birthwt, aes(x=factor(race), y=bwt)) + geom_boxplot() +
  stat_summary(fun.y="mean", geom="point", shape=23, size=10, fill="BLUE")   ###添加平均值到箱线图中
...
#Making a Density Plot of Two-Dimensional Data    ###二位密度图
