...
p = ggplot(heightweight, aes(x=ageYear, y=heightIn, colour=sex)) + 
  geom_point()
p + theme(
  panel.grid.major = element_line(colour="red"),                 ##背景网格主线，红色
  panel.grid.minor = element_line(colour="red", linetype="dashed", size=0.2),      ##背景网格次线，红色，虚线
  panel.background = element_rect(fill="lightblue"),    ##背景颜色
  panel.border = element_rect(colour="blue", fill=NA, size=2))   ##边框颜色

# Options for text items
p + ggtitle("Plot title here") +
  theme(
    axis.title.x = element_text(colour="red", size=14,family="Times",face="bold"),  ###x轴标题
    axis.text.x = element_text(colour="blue"),                                      ###x轴刻度
    axis.title.y = element_text(colour="red", size=14, angle = 90),                 ###y轴标题
    axis.text.y = element_text(colour="blue"),                                      ###y轴刻度
    plot.title = element_text(colour="red", size=20, face="bold"))                  ###题目
